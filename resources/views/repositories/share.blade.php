...
@section('title', 'Zdieľaný repozitár')
...
@section('content')
    ...
    <div class="d-flex align-items-start justify-content-between flex-wrap gap-2 mb-4">
        <div>
            ...
            <div class="text-muted small">Zdieľaný repozitár</div>
            @if ($repository->description)
                <div class="text-muted mt-2">{{ $repository->description }}</div>
            @endif
        </div>

        <a href="{{ route('home') }}" class="btn btn-outline-secondary btn-sm rounded-pill">Späť</a>
    </div>
